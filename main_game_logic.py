...
class Game:
    ...
    def resolve_effect(self, effect: Effect, caster_card_id: str, target_id: str):
        """특정 효과를 해결(실행)합니다."""
    ...
    def resolve_effects_type(self, caster_card_id: str, effect_type: EffectType, target_id: str):
        ...
        if effect_list:
            for effect in effect_list:
                self.resolve_effect(effect, caster_card_id, target_id)
            return True
        return False
    ...
    def _handle_card_effect(self, event: Event, effect_to_resolve: Effect):
        ...
        self.resolve_effect(effect_to_resolve, card_id, target_id)

    def _on_follower_super_evolved(self, event: FollowerSuperEvolvedEvent):
        """초진화 효과 처리"""
        card_id = event.card_id
        effect_trigger_types = [EffectType.ON_SUPER_EVOLVE, EffectType.SUPER_EVOLVED, EffectType.ON_EVOLVE, EffectType.EVOLVED]

        for effect_trigger_type in effect_trigger_types:
            if self.game_state_manager.has_keyword(card_id, effect_trigger_type):
                print(f"[LOG] 초진화 효과 처리: 대상 카드: {self.game_state_manager.get_card_name(card_id)} -> 이펙트 '{effect_trigger_type.name}'")
    ...
    def _on_turn_end(self, event: TurnEndEvent):
        ...
        cards_with_my_turn_end = self.game_state_manager.get_cards_with_keyword(player_id, Zone.FIELD, EffectType.ON_MY_TURN_END)
    ...
    def play_card(self, player_id: str, card_id: str, enhanced_cost=0, use_extra_pp=False):
        ...
        # 카드에 정의된 즉발 효과 해결
        self.event_manager.publish(CardPlayedEvent(player_id=player_id, card_id=card_id, enhanced_cost=enhanced_cost))
        self.process_events()
        self.gui.update()
        return True
